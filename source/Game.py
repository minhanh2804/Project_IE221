...
class Game:
    ...
    """This funciton is used for drawing out screens corresponding to design"""
        ...
        if self.screen_mode == 0:       #logo screen1
            ...
            surface = pg.Surface((WIDTH, HEIGHT))
            bg = pg.image.load(os.path.join(self.img_dir, 'bg.png'))
            surface.blit(bg,(0,0))
            ...
            if self.song_select > 2:
                self.draw_text(self.song_list[self.song_select - 3], 16, BLACK, 0.29 * WIDTH, 0.25 * HEIGHT - 20, max(screen_alpha, 0))

            if self.song_select > 1:
                self.draw_text(self.song_list[self.song_select - 2], 18, BLACK, 0.27 * WIDTH, 0.375 * HEIGHT - 20, max(screen_alpha, 0))

            self.draw_text(self.song_list[self.song_select - 1], 24, BLACK, 0.25 * WIDTH, 0.5 * HEIGHT - 20, screen_alpha)

            if self.song_select < self.song_num:
                self.draw_text(self.song_list[self.song_select], 18, BLACK, 0.27 * WIDTH, 0.625 * HEIGHT - 20, max(screen_alpha, 0))

            if self.song_select < self.song_num - 1:
                self.draw_text(self.song_list[self.song_select + 1], 16, BLACK, 0.29 * WIDTH, 0.75 * HEIGHT - 20, max(screen_alpha, 0))
            ...
            if self.song_highScore[self.song_select - 1] == -1:
                ...
                if self.song_highScore[self.song_select - 1] >= self.song_perfectScore[self.song_select - 1]:
                    ...
                    rotated_surface = pg.transform.rotate(cleartext_surface, 35)
                    rotated_surface.set_alpha(max(screen_alpha - 100, 0))
                    cleartext_rect = rotated_surface.get_rect()
                    cleartext_rect.midtop = (round(0.75 * WIDTH), round(HEIGHT / 2 - 200))
            ...
            self.draw_text(self.load_language(6), 32, BLACK, 0.69 * WIDTH, HEIGHT / 2 + 85, screen_alpha, select_index[1])
        elif self.screen_mode == 4:             #play screen
            surface = pg.Surface((WIDTH, HEIGHT))
            surface.fill(WHITE)
        ...
        if self.game_tick >= self.song_data[self.song_dataIndex][0]:
            if self.song_data[self.song_dataIndex][1] != -1:
                ...
                for shot in range(shot_num):
                    ...
                    obj_shot = Shot.Shot(self, shot_data[0], shot_data[1], shot_data[2], shot_data[3])
